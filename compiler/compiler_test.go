...
func TestCompilerScopes(t *testing.T) {
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 1 {
		t.Errorf("instructions lenfth wrong. got=%d",
...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	...
	for _, tt := range tests {
		...
		bytecoce := compiler.Bytecode()

		err = testInstructions(tt.expectedInstructions, bytecoce.Instructions)
		if err != nil {
			t.Fatalf("testInstructions failed: %s", err)
		}
		err = testConstants(t, tt.expectedConstants, bytecoce.Constants)
		if err != nil {
...
func testConstants(
	...
	for i, constant := range expected {
		...
		case []code.Instructions:
			...
			if !ok {
				return fmt.Errorf("constant %d - not a funcion: %T",
					i, actual[i])
			}
